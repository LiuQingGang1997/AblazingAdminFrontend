...
import { Button, Form, Input, InputNumber, Switch, Modal, Popconfirm, Space, Table, Typography, message, Image, Select, TreeSelect } from 'antd'
...
const Products: React.FC = () => {
  ...
  const fetchDropdownData = async () => {
    try {
      const [brandsRes, scenesRes, typesRes, categoriesRes, seriesRes] = await Promise.all([
      ...
      if (Array.isArray(seriesRes.data)) setProductSeries(seriesRes.data)
    } catch (error) {
  ...
  const buildCategoryTree = (data: any[]) => {
    ...
    const buildTree = (parentId: number, level: number): any[] => {
    ...
    setCategoryTreeData(buildTree(0, 0))
  }

  const filterTreeByBrand = (tree: any[], brandId: number | null): any[] => {
    if (!brandId) return tree
    
    return tree
      .map(node => {
        const filteredChildren = filterTreeByBrand(node.children || [], brandId)
        const isVisible = !node.brandId || node.brandId === brandId
      ...
      { 
        ...
        render: (_: any, record: any) => (
          <Space direction="vertical" size={0}>
            ...
            {record.categoryName && <Typography.Text type="secondary" style={{ fontSize: 12 }}>分类: {record.categoryName}</Typography.Text>}
            {record.seriesName && <Typography.Text type="secondary" style={{ fontSize: 12 }}>系列: {record.seriesName}</Typography.Text>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0 24px' }}>
            <Form.Item name="brandId" label="归属品牌">
              <Select 
                ...
                onChange={(value) => {
                  ...
                  form.setFieldsValue({ categoryId: undefined, seriesId: undefined })
                }}
              />
            </Form.Item>
            <Form.Item name="categoryId" label="所属分类">
              <TreeSelect 
                allowClear 
                placeholder="选择分类"
                treeData={filterTreeByBrand(categoryTreeData, selectedBrandId)}
              />
            </Form.Item>
            <Form.Item name="seriesId" label="所属系列">
              <Select 
                allowClear 
                placeholder="选择系列"
                options={productSeries
                  .filter(s => !selectedBrandId || s.brandId === selectedBrandId || s.brandId === null)
